...
public class SearchArrow {
  ...
  private final double ARROW_SIZE = 0.1f;
  // The circle quad is 40% of the screen width or height, whichever is smaller.
  private final double CIRCLE_SIZE = 0.4f;
  
  // The target position is (1, theta, phi) in spherical coordinates.
  private double mTargetTheta = 0;
  private double mTargetPhi = 0;
  private TexturedQuad mCircleQuad = null;
  private TexturedQuad mArrowQuad = null;
  private double mArrowOffset = 0;
  private double mCircleSizeFactor = 1;
  private double mArrowSizeFactor = 1;
  private double mFullCircleScaleFactor = 1;
  ...
  public void resize(GL10 gl, int screenWidth, int screenHeight, double fullCircleSize) {
  ...
  public void draw(GL10 gl, Vector3 lookDir, Vector3 upDir, SearchHelper searchHelper,
    ...
    double lookPhi = Math.acos(lookDir.y);
    double lookTheta = Math.atan2(lookDir.z, lookDir.x);
    
    // Positive diffPhi means you need to look up.
    double diffPhi = lookPhi - mTargetPhi;
    
    // Positive diffTheta means you need to look right.
    double diffTheta = lookTheta - mTargetTheta;
    ...
    double angle = Math.atan2(diffPhi, diffTheta);
    ...
    double roll = angleBetweenVectorsWithRespectToAxis(new Vector3(0, 1, 0), upDir, lookDir);
    
    angle += roll;
    
    // Distance is a normalized value of the distance.
    double distance = 1.0f / (1.414f * Math.PI) *
        Math.sqrt(diffTheta * diffTheta + diffPhi * diffPhi);
   
    gl.glEnable(GL10.GL_BLEND);
    gl.glBlendFunc(GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA);
    
    gl.glPushMatrix();
    gl.glRotatef((float) (angle * MathUtil.RADIANS_TO_DEGREES), 0, 0, -1);
    ...
    double expandFactor = searchHelper.getTransitionFactor();
    
    if (expandFactor == 0) {
      gl.glColor4x(FixedPoint.ONE, FixedPoint.ONE, FixedPoint.ONE, FixedPoint.ONE);
      
      double redFactor, blueFactor;
      if (nightVisionMode) {
        redFactor = 0.6f;
        blueFactor = 0;
      } else {
      ...
      gl.glTexEnvfv(GL10.GL_TEXTURE_ENV, GL10.GL_TEXTURE_ENV_COLOR, 
                    new float[] {(float) redFactor, 0.0f, (float) blueFactor, 0.0f}, 0);
      
      gl.glPushMatrix();
      float circleScale = (float) mCircleSizeFactor;
      gl.glScalef((circleScale, circleScale, circleScale);
      mCircleQuad.draw(gl);
      gl.glPopMatrix();
    
      gl.glPushMatrix();
      float arrowScale = (float) mArrowSizeFactor;
      gl.glTranslatef((float) (mArrowOffset * 0.5), 0, 0);
      gl.glScalef(arrowScale, arrowScale, arrowScale);
      mArrowQuad.draw(gl);
      gl.glPopMatrix();
    } else {
      gl.glColor4x(FixedPoint.ONE, FixedPoint.ONE, FixedPoint.ONE, 
                   FixedPoint.doubleToFixedPoint(0.7f));
      
      gl.glTexEnvfv(GL10.GL_TEXTURE_ENV, GL10.GL_TEXTURE_ENV_COLOR, 
          new float[] {1, nightVisionMode ? 0 : 0.5f, 0, 0.0f}, 0);
      
      gl.glPushMatrix();
      float circleScale = (float) (mFullCircleScaleFactor * expandFactor +
  ...
  public void setTarget(Vector3 position) {
    ...
    mTargetPhi = Math.acos(position.y);
    mTargetTheta = Math.atan2(position.z, position.x);
  }
  ...
  private static double angleBetweenVectorsWithRespectToAxis(Vector3 v1, Vector3 v2, Vector3 axis) {
    ...
    double cosAngle = VectorUtil.dotProduct(v1proj, v2);
    double sinAngle = -VectorUtil.dotProduct(perp, v2);
